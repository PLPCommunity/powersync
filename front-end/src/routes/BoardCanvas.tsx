...
import { auth } from "../utils/firebase"; // <-- use the same compat auth you already have
...
const isRectLike = (s: Shape): s is RectLike => ["rect","ellipse","diamond","circle","triangle","text","cylinder","cloud","callout","starburst"].includes((s as any).type);
const isLineLike = (s: Shape): s is LineLike => ["line","arrow","arrowDouble","orthogonal"].includes((s as any).type);
...
const rotatePoint = (cx:number,cy:number,x:number,y:number,rad:number) => { const dx=x-cx,dy=y-cy,cos=Math.cos(rad),sin=Math.sin(rad); return { x: cx + dx*cos - dy*sin, y: cy + dx*sin + dy*cos }; };
function toLocalOfRect(s:{x:number;y:number;w:number;h:number;rotation?:number}, x:number, y:number){ const {cx,cy}=getRectCenter(s); const rad=getRotation(s); const dx=x-cx, dy=y-cy; const cos=Math.cos(rad), sin=Math.sin(rad); const lx=dx*cos+dy*sin, ly=-dx*sin+dy*cos; const halfW=Math.abs(s.w)/2, halfH=Math.abs(s.h)/2; return { lx, ly, halfW, halfH }; }
...
function rectHandlesRotated(s: RectLike): HandlePoint[] {
  const rad=getRotation(s); const {cx,cy}=getRectCenter(s); const left=s.x,top=s.y,right=s.x+s.w,bottom=s.y+s.h;
  const corners: HandlePoint[] = ([
    {name:"nw" as const, x:left, y:top},{name:"ne" as const, x:right,y:top},
    {name:"sw" as const, x:left, y:bottom},{name:"se" as const, x:right,y:bottom},
  ]).map(({name,x,y})=>({name, ...rotatePoint(cx,cy,x,y,rad)}));
  const halfH=Math.abs(s.h)/2, offset=24;
  const rx=cx + Math.sin(rad)*(halfH+offset); const ry=cy - Math.cos(rad)*(halfH+offset);
...
export function BoardCanvas() {
  ...
  async function ensureSession() {
    ...
    try {
      ...
      const r = await fetch(`${API_BASE}/api/sessionLogin`, {
        ...
        credentials: "include", // <-- send & set cookie
  ...
  useEffect(() => {
    let mounted = true;
    // connect socket regardless of auth (drawing collab), HTTP routes will be auth-protected
    socketRef.current = io(API_BASE, { transports: ["websocket"] });

    const unsub = auth.onAuthStateChanged(async (u) => {
      if (!mounted) return;
      if (!u) {
        // not signed in -> back to boards/login
        navigate("/boards");
        return;
      }

      // Make sure we have the cookie before hitting protected endpoints
      const ok = await ensureSession();
      if (!ok) {
        // session exchange failed -> bounce out
        navigate("/boards");
        return;
      }

      // Now you can call protected endpoints with credentials: 'include'
      try {
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      try {
        ...
        if (r.status === 401) {
          // lost session -> back to boards (or prompt login)
          navigate("/boards");
        }
      } catch {}
    }, 500);
    return () => clearTimeout(t);
  }, [shapes, boardId, navigate]);
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      try {
        ...
        if (r.status === 401) navigate("/boards");
      } catch {}
    }, 500);
    return () => clearTimeout(t);
  ...
  function renderCanvas() {
    ...
    if (draftShapeRef.current) {
      ctx.save(); ctx.globalAlpha = 0.7; drawShape(ctx, draftShapeRef.current); ctx.restore();
    }
  }
  ...
  function drawShape(ctx: CanvasRenderingContext2D, s: Shape) {
    ...
    if (isRectLike(s)) {
      ...
      if (s.type === "text") {
        ...
        ctx.fillStyle = t.color || "#111111"; ctx.fillText(t.text || "", lx, ly); ctx.restore(); return;
      }
    ...
    if (l.type === "orthogonal") {
      ...
      ctx.beginPath(); ctx.moveTo(l.x1,l.y1); ctx.lineTo(bend.x,bend.y); ctx.lineTo(l.x2,l.y2); ctx.stroke(); return;
    }
    ctx.beginPath(); ctx.moveTo(l.x1,l.y1); ctx.lineTo(l.x2,l.y2); ctx.stroke();
    if (l.type === "arrow" || l.type === "arrowDouble") { drawArrowHead(ctx,l.x1,l.y1,l.x2,l.y2,(l as any).strokeWidth||2); if (l.type === "arrowDouble") drawArrowHead(ctx,l.x2,l.y2,l.x1,l.y1,(l as any).strokeWidth||2); }
  }
  function drawArrowHead(ctx:CanvasRenderingContext2D,x1:number,y1:number,x2:number,y2:number,width:number){ const angle=Math.atan2(y2-y1,x2-x1); const size=10+width; ctx.beginPath(); ctx.moveTo(x2,y2); ctx.lineTo(x2 - size*Math.cos(angle-Math.PI/6), y2 - size*Math.sin(angle-Math.PI/6)); ctx.moveTo(x2,y2); ctx.lineTo(x2 - size*Math.cos(angle+Math.PI/6), y2 - size*Math.sin(angle+Math.PI/6)); ctx.stroke(); }
  function drawCylinder(ctx:CanvasRenderingContext2D,r:RectLike){ const {x,y,w,h}=r; const rx=Math.abs(w/2); const ry=Math.abs(Math.min(h/6, Math.abs(w)/4)); const cx=x+w/2; if (r.fill){ctx.fillStyle=r.fill; ctx.fillRect(x,y+ry,w,h-2*ry);} ctx.strokeRect(x,y+ry,w,h-2*ry); ctx.beginPath(); ctx.ellipse(cx,y+ry,rx,ry,0,0,Math.PI*2); if (r.fill){ctx.fillStyle=r.fill; ctx.fill();} ctx.stroke(); ctx.save(); ctx.setLineDash([6,4]); ctx.beginPath(); ctx.ellipse(cx,y+h-ry,rx,ry,0,Math.PI,0,true); ctx.stroke(); ctx.restore(); ctx.beginPath(); ctx.ellipse(cx,y+h-ry,rx,ry,0,0,Math.PI); ctx.stroke(); }
  function drawCloud(ctx:CanvasRenderingContext2D,r:RectLike){ const {x,y,w,h}=r; const cx=x+w/2, cy=y+h/2, rx=Math.abs(w/2), ry=Math.abs(h/2); const blobs=[{dx:-0.35,dy:0.1,rr:0.5},{dx:-0.05,dy:-0.15,rr:0.6},{dx:0.35,dy:0.0,rr:0.55},{dx:0.1,dy:0.25,rr:0.45},{dx:-0.25,dy:0.25,rr:0.45},]; ctx.beginPath(); for (let i=0;i<blobs.length;i++){ const b=blobs[i]; const bx=cx+b.dx*w, by=cy+b.dy*h, brx=rx*b.rr, bry=ry*b.rr; if (i===0) ctx.ellipse(bx,by,brx,bry,0,0,Math.PI*2); else { ctx.moveTo(bx+brx,by); ctx.ellipse(bx,by,brx,bry,0,0,Math.PI*2);} } if (r.fill){ctx.fillStyle=r.fill; ctx.fill("evenodd");} ctx.stroke(); }
  function drawCallout(ctx:CanvasRenderingContext2D,r:RectLike){ const radius=Math.min(Math.abs(r.w),Math.abs(r.h))*0.12; const tailW=Math.abs(r.w)*0.22; const tailH=Math.abs(r.h)*0.22; const tailSide=r.w>=0?1:-1; const x=r.x,y=r.y,w=r.w,h=r.h; const x2=x+w,y2=y+h; ctx.beginPath(); roundRectPath(ctx,x,y,w,h,radius); const tx0=x2 - tailW*tailSide; const ty0=y2 - radius*2; const tipX=x2 + 0.1*w*tailSide; const tipY=y2 + 0.1*h; ctx.moveTo(tx0,ty0); ctx.lineTo(tipX,tipY); ctx.lineTo(x2 - radius*2*tailSide, y2 - radius); if (r.fill){ctx.fillStyle=r.fill; ctx.fill();} ctx.stroke(); }
  function roundRectPath(ctx:CanvasRenderingContext2D,x:number,y:number,w:number,h:number,r:number){ const rr=Math.min(r, Math.abs(w)/2, Math.abs(h)/2); const signX=Math.sign(w)||1, signY=Math.sign(h)||1; const _x=x,_y=y,_w=Math.abs(w),_h=Math.abs(h); const left=signX>0? _x : _x - _w; const top=signY>0? _y : _y - _h; ctx.moveTo(left+rr,top); ctx.lineTo(left+_w-rr,top); ctx.quadraticCurveTo(left+_w,top,left+_w,top+rr); ctx.lineTo(left+_w, top+_h-rr); ctx.quadraticCurveTo(left+_w, top+_h, left+_w-rr, top+_h); ctx.lineTo(left+rr, top+_h); ctx.quadraticCurveTo(left, top+_h, left, top+_h-rr); ctx.lineTo(left, top+rr); ctx.quadraticCurveTo(left, top, left+rr, top); }
  function drawStarburst(ctx:CanvasRenderingContext2D,r:RectLike){ const spikes=16; const cx=r.x+r.w/2, cy=r.y+r.h/2; const R=Math.hypot(r.w/2,r.h/2)*0.95; const rInner=R*0.45; ctx.beginPath(); for (let i=0;i<spikes*2;i++){ const ang=(i/(spikes*2))*Math.PI*2; const rad=i%2===0? R : rInner; const px=cx+Math.cos(ang)*rad; const py=cy+Math.sin(ang)*rad; if (i===0) ctx.moveTo(px,py); else ctx.lineTo(px,py); } ctx.closePath(); if (r.fill){ctx.fillStyle=r.fill; ctx.fill();} ctx.stroke(); }
  function drawSelection(ctx:CanvasRenderingContext2D,s:Shape){ ctx.save(); ctx.strokeStyle="#3b82f6"; ctx.lineWidth=1; ctx.setLineDash([6,4]); if (isRectLike(s)){ const rad=getRotation(s); const {cx,cy}=getRectCenter(s); const lx=s.x-cx, ly=s.y-cy; ctx.translate(cx,cy); if (rad) ctx.rotate(rad); ctx.strokeRect(lx-2,ly-2,s.w+4,s.h+4); ctx.setLineDash([]); if (nearlyZeroRotation(s)){ const corners=[{x:s.x,y:s.y},{x:s.x+s.w,y:s.y},{x:s.x,y:s.y+s.h},{x:s.x+s.w,y:s.y+s.h}]; for (const c of corners) drawHandle(ctx, c.x-cx, c.y-cy, true); } const halfH=Math.abs(s.h)/2, offset=24; ctx.beginPath(); ctx.moveTo(0,-halfH); ctx.lineTo(0,-halfH - offset); ctx.stroke(); drawRotateHandle(ctx, 0, -halfH - offset); ctx.restore(); } else if (isLineLike(s)){ drawHandle(ctx, s.x1, s.y1); drawHandle(ctx, s.x2, s.y2); } else if (isFreehand(s)){ const b=s.bbox; ctx.strokeRect(b.x-2,b.y-2,b.w+4,b.h+4); for (const h of rectHandles(b)) drawHandle(ctx,h.x,h.y); } ctx.restore(); }
  ...
  function drawRotateHandle(ctx:CanvasRenderingContext2D,x:number,y:number){ ctx.save(); ctx.fillStyle="#ffffff"; ctx.strokeStyle="#3b82f6"; ctx.lineWidth=1.25; ctx.beginPath(); ctx.arc(x,y,7,0,Math.PI*2); ctx.fill(); ctx.stroke(); ctx.restore(); }
  ...
  /** -------------------- Pointer handlers -------------------- */
  function handlePointerDown(e: React.MouseEvent<HTMLCanvasElement>) { /* unchanged main logic */ 
    ...
    if (["rect","ellipse","diamond","circle","triangle","cylinder","cloud","callout","starburst"].includes(tool)) {
      draftShapeRef.current = { id, type:tool as any, x:p.x, y:p.y, w:0, h:0, stroke, strokeWidth, fill: tool==="starburst" ? "#fff6" : undefined, rotation:0 } as RectLike;
      requestNextFrame(); return;
    }
  ...
  function handleDoubleClick(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    const id = generateId();
    const base: TextShape = { id, type:"text", x:p.x, y:p.y, w:120, h:28, stroke:"#000000", strokeWidth:1, fill:"transparent", text:"", fontSize:20, fontFamily:"Inter, Arial", color:"#111111", rotation:0 };
  ...
  function handlePointerUp() {
    ...
    if (isDraggingRef.current && draggedShapeRef.current && mutatedDuringDragRef.current) {
      const finalShape = draggedShapeRef.current; const originalShape = shapes.find((s) => s.id === finalShape.id);
      if (originalShape) {
  ...
  function hitTest(x:number,y:number,list:Shape[]):{shape:Shape;handle:string|null}|null {
    for (let i=list.length-1;i>=0;i--){
      ...
      if (isRectLike(s)){
        ...
        if (l.type==="orthogonal"){ const bend={x:l.x2,y:l.y1}; if (pointToSegmentDistance(x,y,l.x1,l.y1,bend.x,bend.y)<6) return {shape:s,handle:null}; if (pointToSegmentDistance(x,y,bend.x,bend.y,l.x2,l.y2)<6) return {shape:s,handle:null}; }
  ...
  function requestNextFrame(){ if (animationFrameRef.current) cancelAnimationFrame(animationFrameRef.current); animationFrameRef.current = requestAnimationFrame(() => renderCanvas()); }

  /** -------------------- Clipboard + keyboard -------------------- */
  function cloneForPaste(s:Shape):Shape{ const id=generateId(), offset=16; if (isRectLike(s)){ const base=s as RectLike|TextShape; return { ...base, id, x:(base as RectLike).x+offset, y:(base as RectLike).y+offset } as Shape; } else if (isLineLike(s)){ const l=s as LineLike; return { ...l, id, x1:l.x1+offset, y1:l.y1+offset, x2:l.x2+offset, y2:l.y2+offset } as Shape; } else { const f=s as Freehand; const pts=f.points.map(p=>({x:p.x+offset,y:p.y+offset})); const b=computeBBox(pts); return { ...f, id, points:pts, bbox:b } as Shape; } }
  function copySelected(){ const selId=selectedIdRef.current; if (!selId) return; const s=shapesRef.current.find(sh=>sh.id===selId); if (!s) return; clipboardRef.current = JSON.parse(JSON.stringify(s)); }
  function cutSelected(){ copySelected(); deleteSelected(); }
  function pasteClipboard(){ const clip=clipboardRef.current; if (!clip) return; const pasted=cloneForPaste(clip); setShapes(prev=>[...prev,pasted]); setSelectedId(pasted.id); socketRef.current?.emit("shape-create",{boardId,shape:pasted}); }
  function deleteSelected(){ const selId=selectedIdRef.current; if (!selId) return; isDraggingRef.current=false; draggedShapeRef.current=null; setShapes(prev=>prev.filter(s=>s.id!==selId)); setSelectedId(null); socketRef.current?.emit("shape-delete",{boardId,shapeId:selId}); }
  useEffect(()=>{ const isTypingTarget=()=>{ const el=document.activeElement as HTMLElement|null; return textEditor.visible || (el && (el.tagName==="INPUT"||el.tagName==="TEXTAREA"||(el as any).isContentEditable)); }; const onKeyDown=(e:KeyboardEvent)=>{ if (isTypingTarget()) return; const mod=e.metaKey||e.ctrlKey; const key=e.key.toLowerCase(); if (e.key==="Delete"||e.key==="Backspace"){ if (selectedIdRef.current){ e.preventDefault(); deleteSelected(); } return; } if (mod && key==="c"){ if (selectedIdRef.current){ e.preventDefault(); copySelected(); } return; } if (mod && key==="x"){ if (selectedIdRef.current){ e.preventDefault(); cutSelected(); } return; } if (mod && key==="v"){ e.preventDefault(); pasteClipboard(); return; } }; window.addEventListener("keydown", onKeyDown); return ()=>window.removeEventListener("keydown", onKeyDown); }, [textEditor.visible]);

  /** -------------------- Pan / Zoom -------------------- */
  useEffect(()=>{ const el=wrapperRef.current; if (!el) return; const onWheel=(e:WheelEvent)=>{ e.preventDefault(); const cam=cameraRef.current; if (e.ctrlKey){ const rect=(canvasRef.current as HTMLCanvasElement).getBoundingClientRect(); const screenX=e.clientX-rect.left, screenY=e.clientY-rect.top; const worldBefore=screenToWorld({x:screenX,y:screenY}); const zoomIntensity=0.0015; const newScale=clamp(cam.scale*(1 - e.deltaY*zoomIntensity),0.25,3); cam.scale=newScale; cam.tx = screenX - worldBefore.x*newScale; cam.ty = screenY - worldBefore.y*newScale; } else { cam.tx -= e.deltaX; cam.ty -= e.deltaY; } setViewVersion(v=>v+1); requestNextFrame(); }; el.addEventListener("wheel", onWheel, { passive:false }); return ()=>el.removeEventListener("wheel", onWheel as any); }, []);
  const clamp=(v:number,min:number,max:number)=>Math.max(min,Math.min(max,v));

  /** -------------------- UI -------------------- */
  const palette=["#111111","#EF4444","#F59E0B","#10B981","#3B82F6","#8B5CF6","#EC4899","#000000","#FFFFFF"];
  useEffect(()=>{ const onResize=()=>{ if (!canvasRef.current) return; canvasRef.current.width=window.innerWidth; canvasRef.current.height=window.innerHeight-110; renderCanvas(); }; onResize(); window.addEventListener("resize", onResize); return ()=>window.removeEventListener("resize", onResize); }, []);
    ...
    <main className="">
      <div style={{ width:"100vw", height:"100vh", display:"flex", flexDirection:"column" }}>
        <div className="flex justify-between py-2 px-5 bg-green-200">
          ...
          <input value={boardName} onChange={(e)=>setBoardName(e.target.value)} placeholder="Untitled document" style={{ border:"none", fontSize:16, textAlign:"right", outline:"none" }} className="font-semibold" />
        </div>

        {selectedId && (()=>{ const sel=shapes.find(s=>s.id===selectedId); if (!sel) return null;
          const updateSel=(props:Partial<Shape>)=>{ setShapes(prev=>prev.map(s=>s.id===selectedId? ({...s, ...props} as Shape) : s)); socketRef.current?.emit("shape-update",{boardId,shapeId:selectedId,props}); };
          return (
            <div className="bg-white flex justify-center mx-auto gap-4 my-2 ">
              ...
              <label style={{ display:"flex", alignItems:"center", gap:6 }}>
                ...
                <input type="number" min={1} max={14} value={(sel as any).strokeWidth || 2}
                  onChange={(e)=>{ const v=Number(e.target.value)||1; updateSel({ strokeWidth:v } as any); }} style={{ width:64 }} />
              </label>
              ...
              {isRectLike(sel) && (
                <label style={{ display:"flex", alignItems:"center", gap:6, marginLeft:12 }}>
                  ...
                  <input type="number" step={1} value={(sel as RectLike).rotation ?? 0}
                    onChange={(e)=>{ let v=Number(e.target.value)||0; v=((v%360)+360)%360; const props={ rotation:v } as Partial<Shape>;
                  ...
                  <button onClick={()=>{ const curr=(sel as RectLike).rotation ?? 0; const v=((curr-15+360)%360); const props={ rotation:v } as Partial<Shape>;
                      setShapes(prev=>prev.map(s=>s.id===selectedId? ({...s, ...props} as Shape):s));
                      socketRef.current?.emit("shape-update",{boardId,shapeId:selectedId,props});
                    }} style={{ padding:"2px 8px", border:"1px solid #e5e7eb", borderRadius:6 }} title="Rotate -15°">↶ 15°</button>
                  <button onClick={()=>{ const curr=(sel as RectLike).rotation ?? 0; const v=((curr+15)%360); const props={ rotation:v } as Partial<Shape>;
                      setShapes(prev=>prev.map(s=>s.id===selectedId? ({...s, ...props} as Shape):s));
                      socketRef.current?.emit("shape-update",{boardId,shapeId:selectedId,props});
                    }} style={{ padding:"2px 8px", border:"1px solid #e5e7eb", borderRadius:6 }} title="Rotate +15°">↷ 15°</button>
                </label>
        ...
        <div ref={wrapperRef} style={{ position:"relative", flex:1, background: GRID_BG }}>
          <canvas
            ref={canvasRef} width={window.innerWidth} height={window.innerHeight - 110}
            onMouseDown={handlePointerDown} onDoubleClick={handleDoubleClick}
          ...
          />
          {textEditor.visible && (()=>{ const scr=worldToScreen({x:textEditor.x,y:textEditor.y}); return (
            <textarea autoFocus value={textEditor.value} onChange={(e)=>setTextEditor(t=>({...t, value:e.target.value}))}
              onBlur={()=>finalizeTextEditor()} onKeyDown={(e)=>{ if (e.key==="Enter" && !e.shiftKey){ e.preventDefault(); finalizeTextEditor(); } }}
  ...
  function finalizeTextEditor() {
    ...
    if (!value.trim()) {
      const nextList = shapes.filter((s) => s.id !== shape.id); setShapes(nextList);
      socketRef.current?.emit("shape-delete", { boardId, shapeId: shape.id });
      setTextEditor({ visible:false, x:0, y:0, value:"", shapeId:null }); return;
    }
